...
import de.tum.bgu.msm.scenarios.mito7days.TravelDemandGenerator7days;
 ...
 * - sizeOfZonesInAcre
 */
public final class MitoModel7daysTengos {

    private static final Logger logger = Logger.getLogger(MitoModel7daysTengos.class);
    private final String scenarioName;

    private DataSet dataSet;

    private MitoModel7daysTengos(DataSet dataSet, String scenarioName) {
        this.dataSet = dataSet;
        this.scenarioName = scenarioName;
        MitoUtil.initializeRandomNumber();
    }

    public static MitoModel7daysTengos standAloneModel(String propertiesFile, ImplementationConfig config) {
        logger.info(" Creating standalone version of MITO ");
        Resources.initializeResources(propertiesFile);
        MitoModel7daysTengos model = new MitoModel7daysTengos(new DataSetImpl(), Resources.instance.getString(Properties.SCENARIO_NAME));
        model.readStandAlone(config);
        return model;
    }

    public static MitoModel7daysTengos initializeModelFromSilo(String propertiesFile, DataSet dataSet, String scenarioName) {
        logger.info(" Initializing MITO from SILO");
        Resources.initializeResources(propertiesFile);
        MitoModel7daysTengos model = new MitoModel7daysTengos(dataSet, scenarioName);
    ...
    public void run() {
        ...
        TravelDemandGenerator7daysTengos ttd = new TravelDemandGenerator7daysTengos.Builder(dataSet).build();
    ...
    private void readAdditionalData() {
        ...
        //new SocialNetworkCliquesReader(dataSet).read();
    }
