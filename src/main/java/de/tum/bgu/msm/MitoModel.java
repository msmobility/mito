...
public final class MitoModel {
    ...
    public static MitoModel initializeModelFromSilo(String propertiesFile, DataSet dataSet, String scenarioName) {
        ...
        new CsvGzSkimsReader(dataSet).readOnlyTransitTravelTimes();
        new CsvGzSkimsReader(dataSet).readSkimDistancesNMT();
        new CsvGzSkimsReader(dataSet).readSkimDistancesAuto();
    ...
    private void readStandAlone(ImplementationConfig config) {
        ...
        new CsvGzSkimsReader(dataSet).read();
        readAdditionalData();
    }
