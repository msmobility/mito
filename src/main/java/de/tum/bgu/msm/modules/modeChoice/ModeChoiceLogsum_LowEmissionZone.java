...
public class ModeChoiceLogsum_LowEmissionZone extends Module {

    private final static Logger logger = Logger.getLogger(ModeChoiceLogsum_LowEmissionZone.class);

    private final Map<Purpose, ModeChoiceCalculatorLogsum> modeChoiceCalculatorByPurpose = new EnumMap<>(Purpose.class);
    private static Map<Integer, Boolean> evForbidden = new HashMap<>();

    public ModeChoiceLogsum_LowEmissionZone(DataSet dataSet, List<Purpose> purposes) {
...
//        logger.info("Using the mode choice calculators obtained from MID 2017. Register alternative mode choice calculators is desired.");
    ...
    public void registerModeChoiceCalculatorLogsum(Purpose purpose, ModeChoiceCalculatorLogsum modeChoiceCalculator) {
        final ModeChoiceCalculatorLogsum prev = modeChoiceCalculatorByPurpose.put(purpose, modeChoiceCalculator);
        if (prev != null) {
    ...
    static class ModeChoiceByPurpose extends RandomizableConcurrentFunction<Void> {
        ...
        private final ModeChoiceCalculatorLogsum modeChoiceCalculator;
        private int countTripsSkipped;
        private Map<Mode, Map<String, Double>> coef;

        ModeChoiceByPurpose(Purpose purpose, DataSet dataSet, ModeChoiceCalculatorLogsum modeChoiceCalculator) {
