...
public class PlanGeneratorForTesting {
    ...
    void run() throws FileNotFoundException {
        ...
        Activity workActivity = new Activity(ActivityPurpose.W, 8 * 3600, 16 * 3600, new Coordinate(10000, 0));
        PlanUtils.addMainTour(plan, workActivity, new DummyTravelTimesForABM());
        out.println(plan.logPlan(interval_s));

        //mandatory main subtours and stops
        PlanUtils.addStopBefore(plan, new Activity(ActivityPurpose.A, 7.50 * 3600, 7.85*3600, new Coordinate(1500,0)), workActivity, new DummyTravelTimesForABM());
        out.println(plan.logPlan(interval_s));

        PlanUtils.addSubtour(plan, new Activity(ActivityPurpose.O, 12*3600, 12.75 * 3600, new Coordinate(13000,0)), new DummyTravelTimesForABM());
        out.println(plan.logPlan(interval_s));

        //discretionary tours
        Activity shoppingActivity = new Activity(ActivityPurpose.S, 18 * 3600, 18.40 * 3600, new Coordinate(-2000, 0));
        PlanUtils.addMainTour(plan, shoppingActivity, new DummyTravelTimesForABM());
        out.println(plan.logPlan(interval_s));

        //discretionary tour stops
        PlanUtils.addStopAfter(plan, new Activity(ActivityPurpose.R, 18.5*3600, 19.5 * 3600, new Coordinate(-4000, 0)), shoppingActivity, new DummyTravelTimesForABM());
        out.println(plan.logPlan(interval_s));
