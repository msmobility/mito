...
public class ModeChoiceCalculatorImplAggregate implements ModeChoiceCalculatorAggregate {
    ...
    public EnumMap<Mode, Double> calculateProbabilities(Purpose purpose, MitoAggregatePersona persona, MitoZone originZone, MitoZone destinationZone, TravelTimes travelTimes, double travelDistanceAuto, double travelDistanceNMT, double peakHour_s, Map<Mode, Map<String, Double>> coef, Map<Mode, Map<String, Double>> coefCalibr) {
        EnumMap<Mode, Double> utilities = calculateUtilities(
                purpose, persona, originZone, destinationZone, travelTimes
                , travelDistanceAuto, travelDistanceNMT, peakHour_s, coef, coefCalibr);
    ...
    public EnumMap<Mode, Double> calculateUtilities(Purpose purpose, MitoAggregatePersona persona, MitoZone originZone, MitoZone destinationZone, TravelTimes travelTimes, double travelDistanceAuto, double travelDistanceNMT, double peakHour_s, Map<Mode, Map<String, Double>> coef,
                                                    Map<Mode, Map<String, Double>> coefCalib) {
        ...
        for (Mode mode :coef.keySet()){
            //double distance = isMotorized(mode)? travelDistanceAuto : travelDistanceNMT;
            final Map<String, Double> modeCoef = coef.get(mode);
            final Map<String, Double> modeCoefCalib = coefCalib.get(mode);
            double utility = modeCoef.get("intercept");
            ...
            if (destinationZone.getCalibrRegion().equals("muc")) {
                utility += modeCoefCalib.get("muc");
            } else if (destinationZone.getCalibrRegion().equals("nonMuc")) {
                utility += modeCoefCalib.get("nonMuc");
            } else {
                utility += modeCoefCalib.get("mucSurround");
            }
