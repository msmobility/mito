 ...
 * - sizeOfZonesInAcre
 */
public final class MitoMopedModel {

    private static final Logger logger = Logger.getLogger(MitoMopedModel.class);
    private final String scenarioName;

    private DataSet dataSet;

    private MitoMopedModel(DataSet dataSet, String scenarioName) {
        this.dataSet = dataSet;
        this.scenarioName = scenarioName;
        MitoUtil.initializeRandomNumber();
    }

    public static MitoMopedModel standAloneModel(String propertiesFile, ImplementationConfig config) {
        logger.info(" Creating standalone version of MITO ");
        Resources.initializeResources(propertiesFile);
        MitoMopedModel model = new MitoMopedModel(new DataSet(), Resources.instance.getString(Properties.SCENARIO_NAME));
        model.readStandAlone(config);
        return model;
    }

    public static MitoMopedModel initializeModelFromSilo(String propertiesFile, DataSet dataSet, String scenarioName) {
        logger.info(" Initializing MITO from SILO");
        Resources.initializeResources(propertiesFile);
        MitoMopedModel model = new MitoMopedModel(dataSet, scenarioName);
    ...
    public void run() {
        ...
        TravelDemandGeneratorWithMoped ttd = new TravelDemandGeneratorWithMoped.Builder(dataSet).build();
