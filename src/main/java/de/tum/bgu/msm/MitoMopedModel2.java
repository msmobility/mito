 ...
 * - sizeOfZonesInAcre
 */
public final class MitoMopedModel2 {

    private static final Logger logger = Logger.getLogger(MitoMopedModel2.class);
    private final String scenarioName;

    private DataSet dataSet;

    private MitoMopedModel2(DataSet dataSet, String scenarioName) {
        this.dataSet = dataSet;
        this.scenarioName = scenarioName;
        MitoUtil.initializeRandomNumber();
    }

    public static MitoMopedModel2 standAloneModel(String propertiesFile, ImplementationConfig config) {
        logger.info(" Creating standalone version of MITO ");
        Resources.initializeResources(propertiesFile);
        MitoMopedModel2 model = new MitoMopedModel2(new DataSet(), Resources.instance.getString(Properties.SCENARIO_NAME));
        model.readStandAlone(config);
        return model;
    }

    public static MitoMopedModel2 initializeModelFromSilo(String propertiesFile, DataSet dataSet, String scenarioName) {
        logger.info(" Initializing MITO from SILO");
        Resources.initializeResources(propertiesFile);
        MitoMopedModel2 model = new MitoMopedModel2(dataSet, scenarioName);
    ...
    public void run() {
        ...
        TravelDemandGenerator2WithMoped ttd = new TravelDemandGenerator2WithMoped.Builder(dataSet).build();
